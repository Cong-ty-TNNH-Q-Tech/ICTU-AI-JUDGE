...
const ContestManagePage: React.FC = () => {
  ...
  const fetchContests = useCallback(async (p: number = 1) => {
    ...
    } catch { showToast('Khong the tai danh sach cuoc thi.', 'error'); }
  ...
  const handleSave = async (data: ContestCreateRequest | ContestUpdateRequest) => {
    setIsSaving(true);
    try {
      if (editTarget) { await ContestService.updateContest(editTarget.id, data as ContestUpdateRequest); showToast('Da cap nhat cuoc thi.'); }
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await ContestService.deleteContest(deleteTarget.id); showToast('Da xoa cuoc thi.');
      setDeleteTarget(null); await fetchContests(page);
    } catch { showToast('Khong the xoa cuoc thi nay.', 'error'); }
